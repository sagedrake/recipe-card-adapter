...
app.get("/recipes/:id", (req, res) => {
	...
	db.query("SELECT * FROM recipes WHERE id = ?", [recipeId], (err, data) => {
		...
		let getIngredientsPromise = new Promise((resolve, reject) => {
			const recipeId = req.params.id;
			db.query(
				"SELECT amount, unit, ingredient_name  FROM recipe_ingredients WHERE recipe_id = ?",
				[recipeId],
				(err, data) => {
					if (err) reject(err);
		...
		Promise.all([getTagsPromise, getIngredientsPromise]).then((value) => {
...
app.post("/recipes", (req, res) => {
	...
	db.query(q, values, (err, data) => {
		...
		Promise.all([insertTags(id, req.body.tags), insertIngredients(id, req.body.ingredients)])
			.then((values) => {
...
app.delete("/recipes/:id", (req, res) => {
	...
	Promise.all([recipePromise, deleteTags(recipeId), deleteIngredients(recipeId)])
		.then((values) => {
...
app.put("/recipes/:id", (req, res) => {
	...
	let updateRecipePromise = new Promise((resolve, reject) => {
		const q =
			"UPDATE recipes SET `name` = ?, `instructions` = ?, `rating` = ?, `image` = ? WHERE id = ?";
		const values = [req.body.name, req.body.instructions, req.body.rating, req.body.image];
	...
	Promise.all([updateRecipePromise, updateTagsPromise, updateIngredientsPromise])
		.then((values) => {
